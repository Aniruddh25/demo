...
from revoscalepy import rx_set_compute_context, rx_data_step, rx_summary, rx_create_col_info
...
file_path = "./"
LoS_text = RxTextData(file=os.path.join(file_path, "LengthOfStay.csv"), strings_as_factors = True)

col_type_info = rx_create_col_info(LoS_text)

# Upload the table to SQL.
LengthOfStay_sql = RxSqlServerData(table="LengthOfStay", connection_string=connection_string, string_as_factors = True)
...
LengthOfStay_sql2 = RxSqlServerData(table="LengthOfStay", connection_string=connection_string, string_as_factors=True)

col = rx_create_col_info(LengthOfStay_sql2)
colnames = col.keys()
print(colnames)
...
if method == "missing":
    ...
    # drop_view("LoS0", connection_string) <- overwrite = True should be good enough

    LoS0_sql = RxSqlServerData(table="LoS0", connection_string=connection_string, string_as_factors = True)
