...
from revoscalepy import rx_dforest, rx_btrees, rx_predict, RxSqlServerData, rx_get_var_names, rx_create_col_info
...
LoS_Train = RxSqlServerData(sql_query="SELECT eid, {} FROM LoS WHERE eid IN (SELECT eid from Train_Id)".format(
    ', '.join(training_variables)), connection_string=connection_string, string_as_factors = True)
...
LoS_Test = RxSqlServerData(sql_query="SELECT eid, {} FROM LoS WHERE eid NOT IN (SELECT eid from Train_Id)".format(
    ', '.join(training_variables)), connection_string=connection_string, column_info=col_type_info
)
...
forest_prediction_sql = RxSqlServerData(table="Forest_Prediction",string_as_factors = True, connection_string=connection_string)
...
# Make Predictions, then import them into Python.
boosted_prediction_sql = RxSqlServerData(table="Boosted_Prediction", string_as_factors = True, connection_string=connection_string)
...
fast_prediction_sql = RxSqlServerData(table="Fast_Prediction", string_as_factors = True, connection_string=connection_string)
